...
## Execute a job with the `Goldberg` class
...
This code creates a `Goldberg` object, which coordinates our ETL jobs. Jobs are multithreaded, and run in the background by default. So we have to start the threads and then (in this case) we'll wait until it's finished (which is a second or two).

After the `goldberg` has finished, it will contain a `FactCollection` with all the `Fact` objects that it's created in the course of parsing your data sources. You can see these facts by printing them:

```python
print(goldberg.fact_collection.facts)
```
...
```pycypher
goldberg.write_entity_table('file:///path/to/output/square_table.parquet', entity='Square')
```
...
```python
goldberg.write_tables('file:///path/to/directory/for/output/tables'))
```
